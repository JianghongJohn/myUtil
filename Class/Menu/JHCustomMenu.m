...
- (instancetype)initWithDataArr:(NSArray *)dataArr origin:(CGPoint)origin width:(CGFloat)width rowHeight:(CGFloat)rowHeight
{
    ...
    if (self) {
        ...
        _tableView.backgroundColor = [UIColor colorWithRed:0 green:0 blue:0 alpha:0.7f];
...
- (void)dismissWithCompletion:(void (^)(JHCustomMenu *object))completion
{
    ...
    [UIView animateWithDuration:0.2 animations:^{
        weakSelf.alpha = 0;
        weakSelf.tableView.frame = CGRectMake(weakSelf.origin.x +self.viewWidth -LeftToView, weakSelf.origin.y + TopToView, 0, 0);
...
//    [colors[serie] setFill];
    ...
    CGContextMoveToPoint(context,
                         self.origin.x +self.viewWidth-rightToView+LeftToView * 2.5, self.origin.y +TopToView * 0.3);//设置起点
    
    CGContextAddLineToPoint(context,
                            self.origin.x +self.viewWidth-rightToView+LeftToView * 2, self.origin.y +TopToView);
    
    CGContextAddLineToPoint(context,
                            self.origin.x +self.viewWidth-rightToView+LeftToView * 3, self.origin.y +TopToView);
