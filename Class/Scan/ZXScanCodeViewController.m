...
- (void)configUI{
    ...
    CGFloat padding = (SCREENWIDTH - width)/2;
    CGRect scanRect = CGRectMake(padding, SCREENHEIGHT/10, width, width);
    ...
    if (!_scanRectView) {
        ...
        //让它能够实现点击重新启动扫描
        _scanRectView.userInteractionEnabled = YES;
        UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(startScan)];
        [_scanRectView addGestureRecognizer:tap];
    }
    if (!_tipLabel) {
        _tipLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, _scanRectView.bottom+20, SCREENWIDTH, 30)];
